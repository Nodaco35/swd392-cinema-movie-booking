...
export default function SeatSelectionPage() {
  ...
  // whenever the current location key changes (browser back/forward) or the selected
  // showtime changes we need to re‑fetch the seat list; otherwise navigating back from
  // checkout would reuse the old `bookedSeatIds` array and still show the previously
  // attempted seats as occupied.
  useEffect(() => {
    async function load() {
      if (!showtime) return;
      setLoading(true);
      ...
      try {
        const [allSeats, booked] = await Promise.all([
          ...
          fetchBookedSeatIdsForShowtime(showtime.id),
        ]);
        setSeats(allSeats);
    ...
    load();
  }, [showtime, location.key]);

  const seatsByRow = useMemo(() => {
    const grouped = new Map();
    for (const seat of seats) {
      if (!grouped.has(seat.row_label)) {
        grouped.set(seat.row_label, []);
      }
      grouped.get(seat.row_label).push(seat);
    }
  ...
  // clear any previous selection when the showtime changes (e.g. after a booking)
  useEffect(() => {
    setSelectedSeatIds([]);
  }, [showtime, setSelectedSeatIds]);
