...
import { fetchBookedSeatIdsForShowtime, fetchSeatsByAuditorium } from '../api/seats'

export default function SeatSelectionPage() {
  const navigate = useNavigate()
  const { showtime, cinema, selectedSeatIds, setSelectedSeatIds } = useBooking()
  ...
  return (
    ...
    >
      ...
      {showtime && (
        <>
          {error && (
            <div style={{ color: '#ffb3b3', fontSize: 13, marginBottom: '0.5rem' }}>{error}</div>
          )}
          ...
          >
            {seatsByRow.map(([rowLabel, rowSeats]) => (
              ...
              >
                <span style={{ width: 16, fontSize: 12, textAlign: 'right', color: 'var(--muted)' }}>
                  {rowLabel}
                </span>
                {rowSeats.map((seat) => {
